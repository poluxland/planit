...
<html lang="en">
<head>
    <title>Code coverage report for services/pie-stat-getter.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">services</a> pie-stat-getter.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
  function getGroupBy() {
    return isMSSQL(options) ? <span class="branch-0 cbranch-no" title="branch not covered" >[options.sequelize.col(groupByField)] </span>: [ALIAS_GROUP_BY];
  }
&nbsp;
  function formatResults(records) {
    return P.map(records, (record) =&gt; {
      let key;
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (field.type === 'Date') {
<span class="cstat-no" title="statement not covered" >        key = moment(record[ALIAS_GROUP_BY]).format('DD/MM/YYYY HH:mm:ss');</span>
      } else <span class="missing-if-branch" title="if path not taken" >I</span>if (field.type === 'Dateonly' &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >needsDateOnlyFormating)</span> {
        const offsetServer = <span class="cstat-no" title="statement not covered" >moment().utcOffset() / 60;</span>
        const dateonly = <span class="cstat-no" title="statement not covered" >moment.utc(record[ALIAS_GROUP_BY])</span>
          .add(offsetServer, 'h');
<span class="cstat-no" title="statement not covered" >        key = dateonly.format('DD/MM/YYYY');</span>
      } else {
