...
<html lang="en">
<head>
    <title>Code coverage report for adapters/sequelize.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">adapters</a> sequelize.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
  function getTypeForAssociation(association) {
    ...
    const type = attribute ? <span class="branch-0 cbranch-no" title="branch not covered" >new ApimapFieldTypeDetector(attribute, opts).perform() </span>: 'Number';
&nbsp;
    switch (association.associationType) {
<span class="branch-0 cbranch-no" title="branch not covered" >      case 'BelongsTo':</span>
<span class="branch-1 cbranch-no" title="branch not covered" >      case 'HasOne':</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (association.associationType === 'BelongsTo') {
<span class="cstat-no" title="statement not covered" >      fieldNamesToExclude.push(association.identifierField);</span>
    }
&nbsp;
    return schema;
  }
&nbsp;
  const columns = P
    .each(_.values(model.rawAttributes), (column) =&gt; {
      try {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (column.references &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!column.primaryKey)</span> { <span class="cstat-no" title="statement not covered" >return; </span>}
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        Interface.logger.error(`Cannot fetch properly column ${column.field} of model ${model.name}`, error);</span>
      }
    });
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        Interface.logger.error(`Cannot fetch properly association ${association.associationAccessor} of model ${model.name}`, error);</span>
      }
    });
&nbsp;
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (_.keys(model.primaryKeys).length &gt; 1) {
<span class="cstat-no" title="statement not covered" >        isCompositePrimary = true;</span>
<span class="cstat-no" title="statement not covered" >        idField = 'forestCompositePrimary';</span>
      }
&nbsp;
      _.remove(fields, field =&gt;
        _.includes(fieldNamesToExclude, field.columnName) &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!field.primaryKey)</span>;
&nbsp;
      return {
        name: model.name,
        idField,
