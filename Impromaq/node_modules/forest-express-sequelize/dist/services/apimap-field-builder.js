...
function ApimapFieldBuilder(model, column, options) {
  ...
  function getValidations(automaticValue) {
    var validations = []; // NOTICE: Do not inspect validation for autogenerated fields, it would
  ...
  this.perform = function () {
    ...
    var canHaveDynamicDefaultValue = ['Date', 'Dateonly'].indexOf(schema.type) !== -1 || column.type instanceof DataTypes.UUID;
    var isDefaultValueFunction = typeof column.defaultValue === 'function' || canHaveDynamicDefaultValue && _typeof(column.defaultValue) === 'object';
    ...
    if (!_.isNull(column.defaultValue) && !_.isUndefined(column.defaultValue)) {
      ...
      if (isDefaultValueFunction) {
        schema.isRequired = false; // NOTICE: Do not use the primary keys default values to prevent issues with UUID fields
