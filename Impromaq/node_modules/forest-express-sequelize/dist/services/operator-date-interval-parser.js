...
function OperatorDateIntervalParser(value, timezone, options) {
  ...
  this.getIntervalDateFilter = function () {
    ...
    if (match && match[1]) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(match[1], 'days').startOf('day'));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(1, 'days').endOf('day'));
      return condition;
    }
    ...
    if (match && match[1]) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(match[1] - 1, 'days').startOf('day'));
      condition[OPERATORS.LTE] = moment().toDate();
      return condition;
    }
    ...
    if (toDate) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().startOf(periodValue));
      condition[OPERATORS.LTE] = moment().toDate();
      return condition;
    } else {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(duration, period).startOf(periodValue));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(1, period).endOf(periodValue));
      return condition;
    }
  };
  ...
  this.getIntervalDateFilterForPreviousInterval = function () {
    ...
    if (value === PERIODS_TODAY) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(1, 'days').startOf('day'));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(1, 'days').endOf('day'));
      return condition;
    }
    ...
    if (match && match[1]) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(match[1] * 2, 'days').startOf('day'));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(parseInt(match[1], 10) + 1, 'days').endOf('day'));
      return condition;
    }
    ...
    if (match && match[1]) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(match[1] * 2 - 1, 'days').startOf('day'));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(match[1], 'days'));
      return condition;
    }
    ...
    if (toDate) {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(duration, period).startOf(periodValue));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(duration, period));
      return condition;
    } else {
      condition[OPERATORS.GTE] = toDateWithTimezone(moment().subtract(duration * 2, period).startOf(periodValue));
      condition[OPERATORS.LTE] = toDateWithTimezone(moment().subtract(1 + duration, period).endOf(periodValue));
      return condition;
    }
  };
}
