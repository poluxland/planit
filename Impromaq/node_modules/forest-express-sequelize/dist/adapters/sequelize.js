...
module.exports = function (model, opts) {
  ...
  function getTypeForAssociation(association) {
    var attribute = association.target.rawAttributes[association.targetKey];
  ...
  function getSchemaForAssociation(association) {
    var schema = {
      ...
      reference: "".concat(association.target.name, ".").concat(association.foreignKey),
      inverseOf: null
  ...
  var columns = P.each(_.values(model.rawAttributes), function (column) {
    try {
      if (column.references && !column.primaryKey) {
        return;
      }
      ...
      Interface.logger.error("Cannot fetch properly column ".concat(column.field, " of model ").concat(model.name), error);
    }
  });
  var associations = P.each(_.values(model.associations), function (association) {
    try {
      var schema = getSchemaForAssociation(association);
      fields.push(schema);
    } catch (error) {
      Interface.logger.error("Cannot fetch properly association ".concat(association.associationAccessor, " of model ").concat(model.name), error);
    }
  });
  return P.all([columns, associations]).then(function () {
    ...
    _.remove(fields, function (field) {
      return _.includes(fieldNamesToExclude, field.columnName) && !field.primaryKey;
    });
